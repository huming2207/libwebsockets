 ...
 *
 *
 * A helper for running unit tests against combinations of abstract protocols
 * and transports.
 *
...
{
	struct lws_seq_test_sequencer *s = (struct lws_seq_test_sequencer *)user;
	lws_unit_test_packet_t *exp = (lws_unit_test_packet_t *)s->args.tests[s->state].expect_array;
	lws_abs_t test_abs;
